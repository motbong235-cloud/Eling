...
- KHQR deposit តាម ABA PayWay + auto-polling (ឬ QR ដោយដៃ បើគ្មាន API key)
...
ចំណាំ (ABA PayWay — KHMER SYSTEM): ABA PayWay ជាវិធីទូទាត់ស្វ័យប្រវត្តិ (ក្រៅពី Manual QR)
  តាមរយៈ khmer-system.com — POST /aba បង្កើត payment (ត្រឡប់ payment_id + branded
  card image URL), poll តាម /aba-api/check-payment រហូតដល់ status ជា PAID រួចបញ្ចូលលុយចូល Wallet
  ស្វ័យប្រវត្តិ (ដូចគ្នានឹង logic poll_deposit ចាស់)។
...
ចំណាំ (v16): បន្ថែម product ប្រភេទ "📧 Email (Admin ដាក់ដោយដៃ)" ជាជម្រើសទី ២ ក្នុងពេល
  ...
  ABA PayWay និង Manual QR ដោយឯករាជ្យពីគ្នា ដោយមិនចាំបាច់លុប API key ចេញពី env variables ទេ។
...
ចំណាំ (v17): ប្តូរឈ្មោះហាងទៅជា "Eing StoreKh" (STORE_NAME default) និងលុបមុខងារ Bakong KHQR
  ...
  PUBLIC_BASE_URL/CAMRAPID_WEBHOOK_URL, function camrapid_create/camrapid_check/has_auto_bakong,
  Flask route /camrapid-webhook, ប៊ូតុងជ្រើសរើសវិធីទូទាត់ (លែងចាំបាច់ព្រោះសល់តែ ABA PayWay ជាវិធី
  auto តែមួយ), និង /lastqrerror លែងបង្ហាញ CamRapidPay error ទៀត។ ឥឡូវសល់តែ ABA PayWay (auto) +
  Manual QR ជាវិធីទូទាត់។
"""
...
# --- ABA PayWay (តាម KHMER SYSTEM — khmer-system.com) --- deposit ស្វ័យប្រវត្តិ
# ត្រូវការ Profile Key + Merchant ID ដែលយកបានពី khmer-system.com/operator/profile
...
STORE_NAME = os.environ.get("STORE_NAME", "Eing StoreKh")  # ឈ្មោះហាង — hardcode ជា default តែអាច override តាម env
...
# ករណីហាង/subscriber គ្មាន ABA API key ផ្ទាល់ខ្លួន — deposit នឹងប្រើ
...
TR = {
    ...
    "manual_qr_caption": {
        "km": "💰 Deposit <b>${amount:.2f}</b>\n💳 វិធីទូទាត់: <b>QR ផ្ទាល់ខ្លួនរបស់ហាង</b>\n🔖 <code>{ref}</code>\n{note}\n"
              ...
              "⏳ ការបញ្ចូលលុយនឹងចំណាយពេលបន្តិច ព្រោះត្រូវផ្ទៀងផ្ទាត់ដោយ Admin ដោយផ្ទាល់ (មិនមែន auto ទេ)",
        "en": "💰 Deposit <b>${amount:.2f}</b>\n💳 Payment method: <b>Shop's own QR</b>\n🔖 <code>{ref}</code>\n{note}\n"
              ...
              "⏳ This will take a bit longer since it's verified manually by Admin (not automatic)",
        "zh": "💰 充值 <b>${amount:.2f}</b>\n💳 支付方式: <b>店铺专属二维码</b>\n🔖 <code>{ref}</code>\n{note}\n"
              "📱 请扫描下方二维码并转账 <b>${amount:.2f}</b>\n"
              "📸 <b>请在下一条消息中发送付款截图</b>，以便管理员核实并为您的钱包充值\n"
              "⏳ 由于需要管理员人工核实（非自动到账），处理会稍慢一些",
    },
...
# ------------------------------------------------------------------
# ------------------------------------------------------------------
# MANUAL QR DEPOSIT (សម្រាប់ហាង/subscriber ដែលគ្មាន ABA API key ផ្ទាល់ខ្លួន)
# ------------------------------------------------------------------
def load_payment_config():
    return _load(PAYMENT_CONFIG_FILE, {
        ...
        # (ឧ. aba_enabled=True តែគ្មាន ABA_API_KEY នៅតែមិនអាចប្រើបានទេ)
        "aba_enabled": True,
        "manual_enabled": True,
    })
...
# --- បិទ/បើក វិធីទូទាត់ ---
PAYMENT_METHOD_KEYS = ("aba", "manual")


def is_payment_method_enabled(method):
    """True បើវិធីទូទាត់ (aba/manual) មិនទាន់ត្រូវបានបិទដោយ admin តាម
...
# ------------------------------------------------------------------
# HTTP SESSION (shared by ABA PayWay integration below)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# ABA PAYWAY (តាម KHMER SYSTEM — khmer-system.com) — ជម្រើសទូទាត់ស្វ័យប្រវត្តិ
# ------------------------------------------------------------------
_last_aba_error = ""
...
def _build_aba_app_deeplink(data):
    ...
    deeplink) ត្រឡប់វា — ប៉ុន្តែ *តែក្នុងករណីជា http(s):// URL ប៉ុណ្ណោះ*។ Telegram inline
    keyboard URL button ទទួលយកតែ http/https (និង tg://) protocol ប៉ុណ្ណោះ — custom app
    scheme ដូច 'abamobilebank://...' នឹងធ្វើឲ្យ Telegram API បដិសេធសារទាំងមូលភ្លាមៗ
    (ApiTelegramException: Unsupported URL protocol) ដែលធ្វើឲ្យ user មិនទទួលបាន QR សោះ។
    ដូច្នេះលែងបង្កើត custom-scheme deep link ដោយខ្លួនឯងពី raw QR string ទៀតទេ — បើ
    khmer-system.com មិនផ្តល់ https deeplink មកផ្ទាល់ទេ គ្រាន់តែមិនបង្ហាញប៊ូតុងនេះទេ
    (user នៅតែស្កេន QR image ធម្មតាបាន)។"""
...
def build_qr_image(qr_string, amount=None, ref=None, label=None, subtitle=None, expires_min=5, width=720):
    """បង្កើត branded KHQR card → BytesIO (PNG)។ Fallback ទៅ QR ធម្មតាបើមានបញ្ហា។"""
    from PIL import Image, ImageDraw
    try:
        W = width
        HEADER_H = int(W * 0.30)
        ...
        draw.text((pad, int(W * 0.045) + f_title.size + int(W * 0.010)),
                  "Cambodian QR Payment", font=f_sub, fill=_CARD_SUBTITLE)

        badge_txt = "EING STOREKH"
        ...
        if expires_min:
            ...
            y += int(W * 0.03)
        _cx_text(draw, cx, y, "Scan with any KHQR-supported app", f_small, _CARD_MUTED)
        y += int(W * 0.03)
...
# ------------------------------------------------------------------
# ACTIVE AUTO-DEPOSIT TRACKING (ABA PayWay តែប៉ុណ្ណោះ)
# ------------------------------------------------------------------
...
def _handle_deposit_aba(uid, chat_id, amount, user_obj, call=None):
    ...
    if pay_url and not str(pay_url).strip().lower().startswith(("http://", "https://")):
        # ការពារ Telegram ApiTelegramException (Unsupported URL protocol) ដដែលដែលអាចកើតមាន
        # បើ khmer-system.com ត្រឡប់ pay_url ជា custom scheme ដែរ — មិនប្រើវាធ្វើប៊ូតុងទេ
        print(f"[_handle_deposit_aba] pay_url មិនមែន http(s): {pay_url!r} — មិនប្រើដាក់ប៊ូតុងទេ", flush=True)
        pay_url = None
...
def _start_setqr_flow(chat_id):
    ...
    msg = bot.send_message(
        ...
        f"ប្រើសម្រាប់ deposit ករណីហាងគ្មាន ABA API key ផ្ទាល់ខ្លួន (គ្មាន auto-detect) — "
...
def admin_setqr_note_step(message, qr_file_id):
    ...
    bot.send_message(message.chat.id, "✅ បានកំណត់ QR ទូទាត់ដោយដៃរួចរាល់! User នឹងឃើញ QR នេះពេលចុច ➕ បញ្ចូលលុយ (ករណីគ្មាន ABA auto-payment)។")
...
# ------------------------------------------------------------------
# បិទ/បើក វិធីទូទាត់ (ADMIN_BTN_PAYTOGGLE) — admin អាចបិទ/បើក
